        ...
        <div className="space-y-4">
          {groups.map((group) => (
            <Link to={`/group/${group.id}`} key={group.id} className="bg-white rounded-lg shadow-sm p-4 flex items-center">
              <img src={group.imageUrl || group.coverImageUrl}
                alt={group.name} className="w-28 h-28 rounded-md object-cover" />
              <div className="ml-4 flex justify-between flex-col">
                <div>
                  <h2 className="text-lg font-bold text-gray-900">{group.name}</h2>
                  <p className="text-gray-600">Paticipant : {group.memberCount}</p>
                </div>
                <div className="mt-4 flex space-x-2">
                  ...
                  <button className="px-4 py-2 bg-yellow-500 text-white rounded-md text-sm font-medium" onClick={handleEdit(group)} disabled={busyId === Number(group.id)}
                  >Edit</button>
